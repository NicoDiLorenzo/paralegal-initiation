...
import {isAuthorizedEmail,supabase} from "./supabase";
...
function Login(){const[email,setEmail]=useState(""),[sent,setSent]=useState(false),[busy,setBusy]=useState(false),[message,setMessage]=useState("");const send=async()=>{setMessage("");const normalized=email.trim().toLowerCase();if(!await isAuthorizedEmail(normalized)){setMessage("Este correo no tiene acceso al programa.");return}setBusy(true);const{error}=await supabase.auth.signInWithOtp({email:normalized,options:{shouldCreateUser:true,emailRedirectTo:window.location.origin}});setBusy(false);if(error)setMessage("No pudimos enviar el enlace. Intentá nuevamente en un minuto.");else{setSent(true);setMessage("Enlace enviado. Revisá la bandeja de entrada y spam; solo puede usarse una vez.")}};return <main className="login"><section><Brand/><p className="eyebrow">PRIVATE LEGAL ACADEMY</p><h1>Welcome, Victoria.</h1><p>Registrate con tu correo autorizado. Te enviaremos un enlace seguro de acceso; no necesitás contraseña.</p><label>Email<input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="tu@email.com" autoComplete="email" disabled={sent}/></label><Button size="lg" disabled={busy||sent||!email} onClick={send}>{busy?"Please wait…":sent?"Check your email":"Register / sign in"}<ChevronRight/></Button>{message&&<p className={message.startsWith("Enlace enviado")?"auth-success":"error"}>{message}</p>}{sent&&<button className="language-link" onClick={()=>{setSent(false);setMessage("")}}>Use a different email</button>}<div className="login-note"><ShieldCheck/>Your progress syncs securely between devices. Never enter real client information.</div></section></main>}
